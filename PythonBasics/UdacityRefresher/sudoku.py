...
def check_sudoku(puzzle):
    length = len(puzzle)
    numbers_row = list(range(1, length+1))
    numbers_column = list(range(1, length + 1))
    # count = 0
    for i in range(length):
        ...
        for j in range(length):
            ...
            # print('------ j =', j)
            # print(count)
            if type(puzzle[i][j]) != int or puzzle[i][j] > length or puzzle[i][j] <= 0:
                return False
            if puzzle[i][j] in numbers_row:
                # print('if row')
                numbers_row.remove(puzzle[i][j])
            else:
                return False
            if puzzle[j][i] in numbers_column:
                # print('if column')
                numbers_column.remove(puzzle[j][i])
            else:
                return False
        numbers_row = list(range(1, length + 1))
        numbers_column = list(range(1, length + 1))
